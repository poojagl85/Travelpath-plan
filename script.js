onload = function () {
    ...
    bfssolve.onclick = function(){
        temptext.style.display  = "none";
        temptext2.style.display  = "none";
        container2.style.display = "inline";
        network2.setData(solveusingbfs());
    }


    dfssolve.onclick = function(){
        temptext.style.display  = "none";
        temptext2.style.display  = "none";
        container2.style.display = "inline";
    ...
    function createGraph(data , isplane){
        let graph = [];
        for(let i=1;i<=V;i++){
            graph.push([]);
        }
    ...
    function solveData() {
        ...
        // Creating adjacency list matrix grconstaph from question data
        const graph = createGraph(data ,1);
    ...
    function solveusingbfs(){
        ...
        let bfsans = bfs(graph , src-1 , dst-1) ; 
        let new_edgesbfs = [] ; 


        for(let i =1 ; i < bfsans.length ; i++){

            new_edgesbfs.push({arrows : { to : {enabled : true}} , from: bfsans[i-1]+1 , to : bfsans[i]+1 ,  color: 'orange'}) ; 
    ...
    function solveusingdfs(){
        ...
        let dfsans = dfs(graph , src-1 , dst-1) ; 
        let new_edgesbfs = [] ; 
